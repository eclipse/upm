...
try {
    ...
    s1 = s1 + s2;
    SWIG_exception_fail(SWIG_ValueError, s1.c_str());
    ...
    s1 = s1 + s2;
    SWIG_exception_fail(SWIG_OverflowError, s1.c_str() );

} catch (std::out_of_range& e) {
    std::string s1("UPM Out of Range: "), s2(e.what());
    s1 = s1 + s2;
    SWIG_exception_fail(SWIG_IndexError, s1.c_str() );
    ...
    s1 = s1 + s2;
    SWIG_exception_fail(SWIG_RuntimeError, s1.c_str() );
    ...
    s1 = s1 + s2;
    SWIG_exception_fail(SWIG_SystemError, s1.c_str() );

} catch (...) {
    /* catch everything else */
    SWIG_exception_fail(SWIG_UnknownError, "UPM Unknown exception" );
}
}
